# Вам нужно реализовать полезную функцию для вывода списка коллег, которых надо поздравить
# с днём рождения на неделе.
#
# У вас есть список словарей users, каждый словарь в нём обязательно имеет ключи name и
# birthday. Такая структура представляет модель списка пользователей с их именами и днями
# рождения. name — это строка с именем пользователя, а birthday — это datetime объект, в
# котором записан день рождения.
#
...
def get_birthdays_per_week(users: list[dict]) -> dict:
    ...
    for i in users:
        birthday = i.get('birthday', None) # '2004-03-14'
        birthday = datetime.strptime(birthday, '%Y-%m-%d') # 2004
...
def get_beauty_print(dict1):
    string = ''
    for k, v in dict1.items():
        string += f'day: {k}, name: {v}' + '\n'
